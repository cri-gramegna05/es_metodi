...
def run(
    ...
    """Collect from the enabled sources, dedupe and apply the deterministic rules."""
    ...
    console.print(
        ...
        f"dropped: [yellow]{summary.dropped}[/]"
    )
    ...
    if not summary.candidates:
        ...
        return
    table = Table(title="Candidates (rule stage)", header_style="bold")
    for col in ("brand", "pre", "price", "disc%", "items", "sources", "signals"):
    ...
    for c in summary.candidates[:40]:
        ...
        table.add_row(
            ...
            str(c["prescore"]), price,
            f"{c['discount_pct']:.0f}" if c["discount_pct"] is not None else "-",
            str(c["n_items"]), ",".join(c["sources"]), ",".join(c["signals"]),
        )
    console.print(table)
